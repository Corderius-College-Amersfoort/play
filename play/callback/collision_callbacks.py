...
class CollisionCallbackRegistry:
    ...
    def _handle_collision(self, arbiter, space, data):
        ...
        if shape_a.collision_type in self.callbacks[True]:
            for shape_b_id, callback in self.callbacks[True][shape_a.collision_type].items():
                if shape_b_id == shape_b.collision_type:
                    self.shape_registry[shape_a.collision_type]._touching_callback[shape_a.actual_collision_type] = callback

        if shape_b.collision_type in self.callbacks[True]:
            for shape_a_id, callback in self.callbacks[True][shape_b.collision_type].items():
                if shape_a_id == shape_a.collision_type:
                    self.shape_registry[shape_b.collision_type]._touching_callback[shape_b.actual_collision_type] = callback

        return True

    def _handle_end_collision(self, arbiter, space, data):
        shape_a, shape_b = arbiter.shapes

        if shape_a.collision_type in self.callbacks[True]:
            for shape_b_id, callback in self.callbacks[True][shape_a.collision_type].items():
                if shape_b_id == shape_b.collision_type:
                    self.shape_registry[shape_a.collision_type]._touching_callback[shape_a.actual_collision_type] = None

        if shape_b.collision_type in self.callbacks[True]:
            for shape_a_id, callback in self.callbacks[True][shape_b.collision_type].items():
                if shape_a_id == shape_a.collision_type:
                    self.shape_registry[shape_b.collision_type]._touching_callback[shape_b.actual_collision_type] = None

        if shape_a.collision_type in self.callbacks[False]:
            for shape_b_id, callback in self.callbacks[False][shape_a.collision_type].items():
                if shape_b_id == shape_b.collision_type:
                    run_callback(callback, [], [])
        if shape_b.collision_type in self.callbacks[False]:
            for shape_a_id, callback in self.callbacks[False][shape_b.collision_type].items():
                if shape_a_id == shape_a.collision_type:
                    run_callback(callback, [], [])
    ...
    def register(self, sprite_a, shape_a, shape_b, callback, collision_type, begin=True):
        """
        ...
        if shape_b.collision_type in self.callbacks[begin][shape_a.collision_type]:
            raise ValueError(
                f"Callback already registered for {shape_a} and {shape_b}"
            )
